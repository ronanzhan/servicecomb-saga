...
public class CompositeOmegaCallback implements OmegaCallback {
  ...
  public void compensate(TxEvent event) {
    Map<String, OmegaCallback> serviceCallbacks = callbacks.getOrDefault(event.getServiceName(), emptyMap());

    if (serviceCallbacks.isEmpty()) {
      throw new AlphaException("No such omega callback found for service " + event.getServiceName());
    }

    OmegaCallback omegaCallback = serviceCallbacks.get(event.getInstanceId());
    if (omegaCallback == null) {
      LOG.info("Cannot find the service with the instanceId {}, call the other instance.", event.getInstanceId());
